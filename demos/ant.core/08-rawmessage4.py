...
# Initialize driver
stick = antdrv.USB1Driver(antcfg.SERIAL, log=antcfg.LOG) # No debug, too much data
stick.open()
...
msg = antmsg.ChannelIDMessage(device_type=120)
stick.write(msg.encode())
if evm.waitForAck(msg) != msgtypes.RESPONSE_NO_ERROR:
    sys.exit()
...
msg = antmsg.ChannelPeriodMessage(period=8070)
stick.write(msg.encode())
if evm.waitForAck(msg) != msgtypes.RESPONSE_NO_ERROR:
    sys.exit()
